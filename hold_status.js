...
const OUT = "D:/Unreal Engine/Light_n_Shadow/hold_status.json";
...
(async () => {
  ...
  try {
    await rpc("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "hold-status", version: "1" },
    });
    log.status = await call("project", { action: "get_status" });
    log.engine = await call("editor", { action: "get_engine_state" });
    fs.writeFileSync(OUT, JSON.stringify(log, null, 2));
    console.log("OK", JSON.stringify(log.status).slice(0, 500));
  } catch (e) {
