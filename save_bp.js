...
(async()=>{
  ...
  await rpc('initialize',{protocolVersion:'2024-11-05',capabilities:{},clientInfo:{name:'save',version:'1'}});
  await sleep(1000);
  
  // Final compile
  let r=await bp({action:'compile',path:BP,assetPath:BP});
  console.log('Compile:', r.success?'OK':('FAIL '+r.error));
  
  // Save
  r=await bp({action:'save_asset',path:BP,assetPath:BP});
  console.log('Save:', r.success?'OK':('FAIL '+(r.error||r.raw||'').substring(0,200)));
