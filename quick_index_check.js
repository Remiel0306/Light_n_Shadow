...
(async()=>{
 await rpc('initialize',{protocolVersion:'2024-11-05',capabilities:{},clientInfo:{name:'quick-index-check',version:'1.0'}});
 const g=await bp({action:'read_graph_summary',path:BP,assetPath:BP,graphName:GRAPH});
 const forLoop=(g.nodes||[]).find(n=>n.title==='For Loop');
 const getNodes=(g.nodes||[]).filter(n=>n.title==='Get');
 const idxEdges=(g.dataEdges||[]).filter(e=>e.from===forLoop?.id && e.fromPin==='Index');
 const toGetIndex=idxEdges.filter(e=>e.toPin==='Index' && getNodes.some(n=>n.id===e.to));
 console.log('ForLoop:',forLoop);
 console.log('GetNodes:',getNodes.map(n=>({id:n.id,title:n.title,class:n.class})));
 console.log('IndexEdgesFromForLoop:',idxEdges);
 console.log('IndexToGetNodes:',toGetIndex);
 process.exit(0);
