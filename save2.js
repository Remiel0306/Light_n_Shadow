...
(async()=>{
  ...
  await rpc('initialize',{protocolVersion:'2024-11-05',capabilities:{},clientInfo:{name:'save2',version:'1'}});
  // Get valid actions
  const r=await bp({action:'save',path:BP,assetPath:BP});
  console.log('save:',r.success?'OK':(r.raw||r.error||'').substring(0,200));
  kill();process.exit(0);
